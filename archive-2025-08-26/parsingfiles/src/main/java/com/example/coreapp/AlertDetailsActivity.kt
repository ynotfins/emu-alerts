...
class AlertDetailsActivity : AppCompatActivity() {
    ...
    private fun loadDetails(alertId: String) {
        try {
            Logger.time(TAG, "loadDetails($alertId)") {
                // TODO: Implement details loading
                logD("Details loaded for #$alertId")
            }
        } catch (t: Throwable) {
            logE("loadDetails failed for $alertId", t)
        }
    }
}

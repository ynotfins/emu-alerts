...
import com.emualerts.util.logI
...
class MainActivity : AppCompatActivity() {
    ...
    private val crashlytics: FirebaseCrashlytics? by lazy { 
        try { Firebase.crashlytics } catch (e: Exception) { null }
    }
            ...
            Firebase.firestore
                ...
                .addSnapshotListener { snap, err ->
                    ...
                    docs.take(3).forEachIndexed { idx, d ->
                        Logger.d(TAG, "probe: doc[$idx] id=${d.id} keys=${com.emualerts.util.FirestoreProbe.keys(d)}")
                        Logger.json(TAG, com.emualerts.util.FirestoreProbe.json(d))
                    }
                    // === end probe ===
    ...
    override fun onOptionsItemSelected(item: android.view.MenuItem): Boolean =
        when (item.itemId) {
            R.id.action_test_crash -> {
                ...
                if (BuildConfig.DEBUG) {
                    throw IllegalStateException("Test crash (debug only)")
                } else {
                    Toast.makeText(this, "Test crash only available in debug builds", Toast.LENGTH_SHORT).show()
                }
            }
            R.id.action_refresh -> {
